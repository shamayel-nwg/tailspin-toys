...
- The comment must state the function's purpose, document every parameter with `@param` (including the injectable `db` argument), and document the return value with `@returns`. Keep the description focused on intent and behavior; do not paraphrase the implementation.
