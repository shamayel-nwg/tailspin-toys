...
The homepage catalog can be filtered with the category checkboxes and publisher
selector. Filter selections are reflected in the URL, so filtered views can be
...
The repository's coding standards are maintained in the [Copilot instructions](.github/copilot-instructions.md) and the technology-specific [instruction files](.github/instructions/). They define the shared comment policy (document intent and decisions, not mechanics), TSDoc/JSDoc requirements for exported data-layer functions, documented `Props` contracts for reusable Astro components, and TypeScript formatting rules enforced by ESLint.
